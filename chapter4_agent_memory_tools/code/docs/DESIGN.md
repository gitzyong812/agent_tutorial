...
本文档面向后续开发者，说明 `agent_book/chapter4_agent_memory_tools/code` 当前代码库的已有功能、整体技术架构、文件组织方式，以及“功能模块”和“技术实现”的对应关系。第 4 章的 Agent 增量细节见 `docs/DESIGN_Agent.md`。
...
4. ReActAgent 如何通过原生 Tool Calling 形成决策、行动、观察和再决策循环。
5. 日级记忆如何自动形成，并经过手工巩固成为可检索的核心记忆。

## 1. 项目定位

本项目是第 4 章“配备工具和记忆的 Agent 数字员工”的配套代码。它保留第 2 章 ChatBot 和第 3 章 RAG 的能力，并进一步增加 ReAct 循环、预设与 HTTP 工具、执行轨迹以及两层长期记忆。
...
| 工具管理 | 查看预设工具，维护自定义 HTTP 工具 | `routers/tools.py`、`tools/` | `static/js/tools.js` |
| 记忆管理 | 管理日级与核心记忆，触发记忆巩固 | `routers/memories.py`、`memory/` | `static/js/memories.js` |
| 模型配置 | 配置和测试 OpenAI 兼容对话模型 | `routers/model_configs.py`、`llm.py` | `static/js/models.js` |
| 数字员工 | 配置 ChatBot、RAG 或 ReActAgent，设置提示词、模型、工具和记忆参数 | `routers/agents.py`、`models.py` | `static/js/agents.js` |
...
| httpx | 执行自定义 HTTP 工具 | 支持明确的超时、重定向和响应边界 |
| jsonschema | 校验工具定义和调用参数 | 让 Function Calling 的 Schema 同时约束模型和程序 |
...
4. 注册模型配置、数字员工、对话、知识库、工具和记忆六组路由。
5. 把 `app/static` 挂载到根路径，返回前端页面。

当前项目没有使用数据库迁移工具。由第 3 章代码升级时必须删除旧 `chatbot.db` 后重启，让系统重新建表和写入演示数据。长期维护或生产化时，应增加 Alembic 一类迁移机制。
...
| `docs/DESIGN.md` | 面向开发者，说明项目整体设计和代码结构 |
| `docs/DESIGN_Agent.md` | 说明第 4 章 ReAct、工具和记忆增量 |
| `docs/archives/DESIGN_RAG.md` | 归档的第 3 章 RAG 增量说明 |
...
| `app/models.py` | ORM 数据模型，包括模型、数字员工、会话、知识库、工具和记忆表 |
...
| `routers/tools.py` | 预设工具列表与自定义 HTTP 工具 CRUD |
| `routers/memories.py` | 日级/核心记忆 CRUD 与手工巩固 |
...
| `runners/react.py` | 原生 Tool Calling 循环、工具观察回填和最大步骤控制 |
...
| 文件 | 功能 |
| --- | --- |
| `tools/registry.py` | 预设工具、自定义 HTTP 工具加载、Schema 校验与统一执行 |
| `memory/service.py` | 日级记忆提取、核心记忆巩固、范围隔离与混合检索 |

### 4.7 前端文件

| 文件 | 功能 |
| --- | --- |
| `static/index.html` | 前端页面骨架，包含左侧导航和六个页面容器 |
...
| `static/js/tools.js` | 预设工具展示和自定义 HTTP 工具配置 |
| `static/js/memories.js` | 记忆筛选、手工维护和巩固操作 |
...
- ReActAgent 依赖模型原生 Tool Calling，不支持时不会退回提示词 JSON 协议。
- 自定义 HTTP 工具没有生产级 SSRF 防护、密钥托管和权限审批。
- 自动记忆提取会增加一次模型调用，核心记忆只在用户手工触发时巩固。
...
| Agent 没有调用工具 | `runners/react.py`、Agent 的工具绑定、模型是否支持 Tool Calling |
| 工具参数或 HTTP 调用失败 | `tools/registry.py`、聊天页执行轨迹 |
| 刷新后执行轨迹消失 | `ChatMessage.extra.agent_trace`、`chat.js` 的 `traceHtml` |
| 没有形成日级记忆 | `memory/service.py`、`memory_enabled`、自动提取日志 |
| 记忆检索不到 | 记忆范围、当前 Agent、embedding 配置和关键词是否匹配 |
...
2. 三类运行时分离。ChatBot、RAG 和 ReActAgent 分别集中在独立 Runner 中。
...
7. 记忆与知识分离。知识来自外部资料，记忆来自 Agent 自身经历，两者可以复用检索技术但不混用数据来源。
