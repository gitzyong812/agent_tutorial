...
const AgentsPage = {
  async render() {
    ...
    const [agents, models, tags, tools] = await Promise.all([
  ...
  openForm(a = null) {
    const d = a || {
      ...
      max_steps: DEFAULT_AGENT_MAX_STEPS, memory_enabled: true,
    };
    ...
    const toolRows = [...(this.tools || [])]
      .sort((left, right) => Number(bindingExtras.has(right.id)) - Number(bindingExtras.has(left.id)) || left.id - right.id)
      .map((tool) => {
        ...
        <div class="modal-body">
          <div class="form-grid">
            ...
            <div id="f-agent-section" class="rag-section">
              <div class="field"><label>${App.t("field_tools")}</label><div class="tool-binding-list" id="f-tools">${toolRows}</div></div>
              <div class="form-grid cols-2">
                <div class="field"><label>${App.t("field_max_steps")}</label><input id="f-steps" type="number" min="1" max="${MAX_AGENT_MAX_STEPS}" value="${d.max_steps || DEFAULT_AGENT_MAX_STEPS}"></div>
                <label class="chk field"><input id="f-memory" type="checkbox" ${d.memory_enabled !== false ? "checked" : ""}> ${App.t("field_memory_enabled")}</label>
              </div>
              <p class="muted memory-help">${App.t("memory_enabled_help")}<br>${App.t("memory_search_help")}</p>
            </div>
    ...
    mask.querySelectorAll("[data-config-tool]").forEach((button) => {
      button.onclick = () => {
        ...
        this.sortToolRows(mask.querySelector("#f-tools"));
    ...
    mask.querySelector("#f-save").onclick = async () => {
      ...
      const payload = {
        ...
        max_steps: parseInt(mask.querySelector("#f-steps").value, 10) || DEFAULT_AGENT_MAX_STEPS,
  ...
  sortToolRows(container) {
    [...container.children]
      .sort((left, right) => {
        const leftInput = left.querySelector("[data-tool-id]");
        const rightInput = right.querySelector("[data-tool-id]");
        return Number(rightInput.checked) - Number(leftInput.checked)
          || parseInt(leftInput.dataset.toolId, 10) - parseInt(rightInput.dataset.toolId, 10);
      })
