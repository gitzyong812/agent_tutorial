...
本文档记录 `agent_book/chapter6_multi_agent_collaboration/code` 当前完整代码库的设计。系统从基础 ChatBot 逐步扩展到 RAG、ReActAgent、工具、记忆、Harness 和多智能体协作，在当前目录中共同组成一个可以直接运行的教学系统。
...
6. 多智能体协作展示团队上下文、角色隔离、动态任务规划和依赖图并行执行。
...
团队协作页面通过独立的团队会话接口进入任务规划器。规划器生成带 `depends_on` 的任务图，执行器按拓扑分批运行数字员工，并将各成员的流式结果写回团队共享消息流。
...
| 团队协作 | 团队、共享上下文、任务规划、依赖图执行 | `routers/group_chat.py`、`group_chat/` | `static/js/group_chat.js` |
...
| 团队协作 | `group_conversations`、`group_conversation_members`、`group_chat_messages`、`group_memories`、`group_files` |

已有章节的业务表保持原字段不变，第 5 章通过新增关联表和运行表扩展能力。应用启动时执行 `Base.metadata.create_all`，可为已有 SQLite 数据库补齐新表。

## 8. API 与 SSE

API 按资源划分为模型、数字员工、单聊会话、团队会话、通道、知识库、工具、记忆、技能和 Harness 接口。团队协作使用 `/api/group-conversations` 管理成员、共享环境与消息，并以命名 SSE 返回各任务的执行过程。
...
- 写操作暂停、批准、拒绝、连续确认和幂等。
- 多通道请求、审计查询和敏感字段脱敏。
- 团队成员管理、共享上下文、任务规划、依赖图分批和团队消息 SSE。
