...
def delete_config(config_id: int, db: Session = Depends(get_db)):
    obj = _get_or_404(db, config_id)
    _ensure_chat_config(obj)
    if db.query(models.AgentConfig).filter(models.AgentConfig.model_config_id == config_id).first():
        raise HTTPException(status_code=400, detail="该模型仍被数字员工使用，不能删除")
    db.delete(obj)
    db.commit()
    return {"ok": True}
...
def set_active(config_id: int, payload: schemas.ActiveIn, db: Session = Depends(get_db)):
    ...
    if not payload.is_active:
        published = db.query(models.AgentConfig).filter(
            models.AgentConfig.model_config_id == config_id,
            models.AgentConfig.status == "published",
        ).first()
        if published:
            raise HTTPException(status_code=400, detail="该模型仍被已发布数字员工使用，不能停用")
