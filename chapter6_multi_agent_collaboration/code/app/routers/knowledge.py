...
def delete_tag(tag_id: int, db: Session = Depends(get_db)):
    ...
    # Agent 使用 JSON 保存标签 id，删除标签时同步清理，避免残留无效配置。
    for agent in db.query(models.AgentConfig).all():
        if tag_id in (agent.knowledge_tag_ids or []):
            agent.knowledge_tag_ids = [value for value in agent.knowledge_tag_ids if value != tag_id]
    db.delete(obj)
    db.commit()
    return {"ok": True}
