...
class GroupConversationIn(BaseModel):
    ...
    agent_config_ids: list[int] = Field(default_factory=list)


class GroupAgentMemberIn(BaseModel):
    agent_config_id: int
...
class GroupMembersOut(BaseModel):
    agents: list[GroupMemberOut] = Field(default_factory=list)
    available_agents: list[AgentConfigOut] = Field(default_factory=list)
...
class GroupMessageOut(BaseModel):
    ...
    extra: dict = Field(default_factory=dict)
    agent: AgentConfigOut | None = None
    created_at: datetime
...
class GroupEnvironmentOut(BaseModel):
    memories: list[GroupMemoryOut]
    files: list[GroupFileOut]


class GroupStandardRequest(BaseModel):
    group_id: int
    content: str = Field(min_length=1)
    mentioned_agent_ids: list[int] = Field(default_factory=list)
