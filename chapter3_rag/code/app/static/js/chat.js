...
const ChatPage = {
  ...
  sessionItem(s) {
    ...
    const typeLabel = info && info.agent_type === "rag_chatbot"
      ? App.t("agent_type_rag")
      : App.t("agent_type_chatbot");
    const meta = info
  ...
  async openSession(id) {
    ...
    main.innerHTML = `
      ...
      <div class="chat-messages" id="msgs">
        ${messages.map((m) => this.bubble(m.role, m.content, m.extra?.rag_sources)).join("")}
      </div>
  ...
  bubble(role, content, sources) {
    if (role !== "assistant") {
      return `<div class="msg ${role}">${this.bubbleInner(role, content, sources)}</div>`;
    }
    return `<div class="msg-wrap assistant-wrap">
      <div class="msg assistant" data-raw="${escapeAttr(content)}">${this.bubbleInner(role, content, sources)}</div>
      ${this.copyActionHtml()}
    </div>`;
  },

  bubbleInner(role, content, sources) {
    const body = role === "assistant" ? renderMarkdown(content) : escapeHtml(content);
    return `<div class="msg-content">${body}</div>${this.sourcesHtml(sources)}`;
  },
    ...
    while (true) {
      ...
      for (const evt of events) {
        ...
        if (parsed.event === "done") {
          this.renderAssistant(holder, answer, sources);
          return;
        }
        if (parsed.event === "sources") {
          try {
            sources = JSON.parse(parsed.data);
            this.renderAssistant(holder, answer || App.t("chat_thinking"), sources);
          } catch (_) {
            sources = null;
          }
          continue;
  ...
  renderAssistant(holder, content, sources) {
    const raw = content === App.t("chat_thinking") ? "" : content;
    holder.dataset.raw = raw;
    holder.innerHTML = this.bubbleInner("assistant", content, sources);
  },
