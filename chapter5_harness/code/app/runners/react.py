    ...
    def run(
        ...
        *,
        ...
        with SessionLocal() as db:
            ...
            if channel == "weixin" and explicit_query is not None:
                final = "微信通道不支持创建技能，请回到网页对话中完成。"
        ...
        while state["step"] <= max_steps:
            ...
            while state.get("call_index", 0) < len(calls):
                ...
                if error_result is not None:
                    ...
                    execution = (
                        ToolExecution(False, {"error": "微信通道不支持创建技能，请回网页完成"})
                        if state.get("channel") == "weixin"
...
def build_agent_prompt(agent: models.AgentConfig, language: str, *, channel: str = "web") -> str:
    ...
    write_rule = (
        "当前为微信通道，不提供需要授权的写入工具。任务需要写操作时，明确说明限制并请用户回网页完成。"
        if channel == "weixin"
