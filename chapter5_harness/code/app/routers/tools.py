...
def _tool_out(item: models.ToolConfig) -> dict:
    return {
        ...
        "risk_level": item.policy.risk_level if item.policy else (
            "read" if item.tool_type == "builtin" else "write"
        ),
...
def _split_policy(data: dict) -> tuple[dict, str]:
    data = dict(data)
    return data, data.pop("risk_level", "write")
...
def create_tool(payload: schemas.ToolConfigIn, db: Session = Depends(get_db)):
    data, risk_level = _split_policy(_validate(payload, db))
    item = models.ToolConfig(tool_type="http", **data)
    item.policy = models.ToolPolicy(risk_level=risk_level)
    db.add(item)
    db.commit()
    db.refresh(item)
...
def update_tool(tool_id: int, payload: schemas.ToolConfigIn, db: Session = Depends(get_db)):
    ...
    data, risk_level = _split_policy(_validate(payload, db, tool_id))
    for key, value in data.items():
        setattr(item, key, value)
    if item.policy is None:
        item.policy = models.ToolPolicy(risk_level=risk_level)
    else:
        item.policy.risk_level = risk_level
    db.commit()
    db.refresh(item)
    return _tool_out(item)
