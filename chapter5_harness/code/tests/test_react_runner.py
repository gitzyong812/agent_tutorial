...
from app.runners.react import ReactRunner, build_agent_prompt
...
def test_tool_completion_yields_text_before_model_stream_finishes(agent, monkeypatch):
    finished = False
    ...
    events = llm.stream_with_tools(agent, [{"role": "user", "content": "回答"}], [])
    assert next(events) == {"kind": "text_delta", "content": "第一段"}
    assert not finished
    remaining = list(events)
    assert finished
    assert remaining[0] == {"kind": "text_delta", "content": "第二段"}
    assert remaining[-1]["content"] == "第一段第二段"


def test_agent_prompt_allows_focused_questions_when_user_invites_discussion(agent):
    prompt = build_agent_prompt(agent, "zh")
    assert "以下任一情况可以调用 ask_human" in prompt
    assert "不清楚的地方可以讨论" in prompt
    assert "有问题可以问我" in prompt
    assert "会实质影响任务方向、关键选择或结果质量" in prompt
    assert "即使用户允许追问，也应先利用已有上下文和工具" in prompt
    assert "不得为了完善方案逐项追问用户" in prompt
    assert f"最多调用 {config.MAX_HUMAN_REQUESTS_PER_RUN} 次" in prompt
...
def test_react_stops_at_max_steps(db, agent, monkeypatch):
    ...
    _mock_tool_stream(monkeypatch, always_tool)
    final_messages = []

    def final_summary(agent_obj, messages):
        final_messages.extend(messages)
        yield "达到上限后的总结"

    monkeypatch.setattr(llm, "stream_chat", final_summary)
